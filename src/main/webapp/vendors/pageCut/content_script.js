...
function bindCenter(){
  ...
  function captureSelection() {
    ...
    if (initScrollTop>selTop){
      ...
      removeSelection();
      setTimeout(function(){
    	  sendRequest({action:"capture_selected_done",data:{x:offsetX,y:offsetY,w:selW,h:selH}})}, 100);
      captureSelected({action:"capture_selected_done",data:{x:offsetX,y:offsetY,w:selW,h:selH}});
    }
  }
...
var wrapperHTML =
  ...
  '      <a id="awesome_screenshot_cancel"><span id="awesome_screenshot_cancel_icon"></span>取消</a>' +
  '      <a id="awesome_screenshot_capture"><span id="awesome_screenshot_capture_icon"></span>提交</a>' +
  '    </div>' +
  '  </div>' +
  '</div>';
