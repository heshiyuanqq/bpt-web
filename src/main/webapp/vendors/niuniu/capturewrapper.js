...
function PluginLoadedCallback(success)
{
    ...
    {
        $('#imgshow').hide();
	    $('#imgshow').attr('src', "loading.gif?v=1");
	    $('#btnReload').hide();
        $('#btnCapture').show();
    }
}
...
function ShowIntallDownload()
{
    ...
    $('#btnCapture').hide();
	var date = new Date();
    $('#downCapture').attr('src', getRootPath()+"/downloadniuniu/CaptureInstall.exe?" + date.getMinutes() + date.getSeconds());
    $('#downloadNotice').show();
}
...
function ShowChromeInstallDownload()
{
    ...
    //$('#downaddr').attr('href', "../../downloadniuniu/CaptureInstallChrome.exe");
    $('#downCapture').attr('src', getRootPath()+"/downloadniuniu/CaptureInstallChrome.exe?" + date.getMinutes() + date.getSeconds());
    $('#downloadNotice').show();
    $('#btnReload').show();
    $('#btnCapture').hide();
}
...
function ReloadPlugin() 
{
    ...
    $('#btnReload').hide();
     ...
     else
     {
        ...
        {
            //about:addons
            browserInfo = "请检查浏览器地址拦下是否有询问是否启用控件的提示，如：<img src=\"ffnotice.png\" border=0 />，如果有，则允许控件运行；";
        ...
        //else if(brow.opera) {bInfo="Opera "+brow.version;}
         ...
         + '<br/>3. 刷新页面或重新启动浏览器试下<br/>4. 如果仍旧不能截图，请联系我：');
     }
}
