...
/**
 * 对数组进行排序（可指定是否降序及比较函数） srcArray, bDesc, compFunc(elA, elB)
 */
function sortArray() {
	var args = arguments;
	var xArray = args[0];
...
/**
 * 对数组进行排序（可指定是否降序及比较函数） bDesc, compFunc(elA, elB)
 */
...
if (Date._parse == null) {
	...
	//
	Date.isValidDate = function(datStr) {
		return !isNaN(Date.parse(datStr, true));
	};
};
...
function merge(original, overwrite) {
	original = original || {};
	overwrite = overwrite || {};
	var keyMap = KeyMap.from(overwrite);
	var keys = keyMap.keys();
	for ( var i = 0; i < keys.length; i++) {
		var key = keys[i];
		original[key] = keyMap.get(key);
	}
	return original;
}
...
/**
 * 比较两个数组，得到 theArray 相对于 refArray 的变化结果：</br> more : 多出的元素列表</br> less : 减少的元素列表</br> same : 一样的的元素列表（根据eqlFunc判断）</br> different : 不同的元素列表（根据eqlFunc判断）</br>
 ...
 * 已经提供了默认的根据给定的id列名称进行比较的函数）<br/> recEqlFunc : 判断两条记录是否相等（无变化）的函数，<br/> 已经提供了默认的判断两条记录相等比较的函数）<br/>
 */
...
function isDigitStr(checkStr) {
	var regExp = /^(\d)+$/;
	return regExp.test(checkStr);
}

function isDigitOrHyphenStr(checkStr) {
	var regExp = /^(\d|-)+$/;
	return regExp.test(checkStr);
}
...
/**
 * 重复单击检查类 repeatClickChecker.isValidFor("log") <br/> repeatClickChecker.isValidFor("log", 500)
 */
function RepeatClickChecker() {
...
// 全局预定义对象
var repeatClickChecker = new RepeatClickChecker();
//
// ------------------------------------------------------------------
//
/**
...
//
function notifyHostPageLoad(extra) {
	if (hasHostWindow()) {
		var hostWin = getHostWindow();
		var hostDialogMonitor = hostWin.dialogMonitor;
		hostDialogMonitor.notifyPageLoad(getPageName(), extra);
	}
}
function getThisPageArgs() {
	if (hasHostWindow()) {
		var hostWin = getHostWindow();
		var hostDialogMonitor = hostWin.dialogMonitor;
		return hostDialogMonitor.getPageArgs(getPageName());
	}
	return null;
}
function notifyHostPageResult(result) {
	if (hasHostWindow()) {
		var hostWin = getHostWindow();
		var hostDialogMonitor = hostWin.dialogMonitor;
		hostDialogMonitor.notifyPageResult(getPageName(), result);
	}
}
function getThisPageDialog() {
	if (hasHostWindow()) {
		var hostWin = getHostWindow();
		var hostDialogMonitor = hostWin.dialogMonitor;
		return hostDialogMonitor.getPageDialog(getPageName());
	}
	return null;
}
// ------------------------------------------------------------------
/**
 * onPageLoad(pageName, extra); onPageResult(result)
 */
var dialogMonitor = {
	pageDialogs : {},
	setPageDialog : function(pageName, dialog) {
		this.pageDialogs[pageName] = dialog;
	},
	getPageDialog : function(pageName) {
		return this.pageDialogs[pageName];
	},
	//
	pageArguments : {},
	setPageArgs : function(pageName, args) {
		this.pageArguments[pageName] = args;
	},
	getPageArgs : function(pageName) {
		return this.pageArguments[pageName];
	},
	//
	pageEventHandlers : {},
	addPageHandler : function(pageName, pageHandler) {
		this.pageEventHandlers[pageName] = pageHandler;
	},
	removePageHandler : function(pageName) {
		delete this.pageEventHandlers[pageName];
	},
	notifyPageLoad : function(pageName, extra) {
		var pageHandler = this.pageEventHandlers[pageName];
		if (pageHandler != null) {
			var loadHanlder = pageHandler["onPageLoad"];
			if (typeof loadHanlder == "function") {
				loadHanlder(pageName, extra);
			}
		}
	},
	notifyPageResult : function(pageName, result) {
		var pageHandler = this.pageEventHandlers[pageName];
		if (pageHandler != null) {
			var resultHanlder = pageHandler["onPageResult"];
			if (typeof resultHanlder == "function") {
...
// 下载给定的Url资源到指定的页面目标(通常是一个隐藏的iframe)中
function downloadFromUrl(url, target) {
	var linkId = "_unique_dowload_url_link";
	var linkObj = document.getElementById(linkId);
	if (linkObj == null) {
		linkObj = document.createElement("A");
		linkObj = document.body.appendChild(linkObj);
		linkObj.id = linkId;
...
function addStyleClass(ctrl, className) {
	if (isString(ctrl)) {
	...
	var orgClass = ctrl.className;
	if (orgClass == null) {
		orgClass = "";
	}
	var tmpClass = replaceRegStr(orgClass, "(\\s)+", ",");
	var classNames = tmpClass.split(",");
	if (classNames.indexOf(className) == -1) {
		classNames.append(className);
	}
	ctrl.className = classNames.join(" ").trim();
}

function removeStyleClass(ctrl, className) {
	if (isString(ctrl)) {
		ctrl = document.getElementById(ctrl);
	}
	if (ctrl == null) {
...
function replaceStyleClass(ctrl, oldClassName, newClassName, addIfNone) {
	if (isString(ctrl)) {
		ctrl = document.getElementById(ctrl);
	}
	if (ctrl == null) {
	...
	var tmpClass = replaceRegStr(orgClass, "(\\s)+", ",");
	var classNames = tmpClass.split(",");
	var oldIndex = classNames.indexOf(oldClassName);
	if (oldIndex == -1) {
		if (addIfNone == true && newClassName != null && !newClassName.isBlank()) {
		...
		if (newClassName == null || newClassName.isBlank()) {
			...
			classNames[oldIndex] = newClassName;
		}
	}
	ctrl.className = classNames.join(" ").trim();
}
...
function loadSelectData(selId, json, config, defValue) {
	...
	//
	var optCount = json == null ? 0 : json.length;
	var valueField = config.valueField == null ? "value" : config.valueField;
	var textField = config.textField == null ? "text" : config.textField;
	var unSelectedVal = config.unSelectedVal == null ? "" : config.unSelectedVal;
	var retIndex = -1;
	...
	for ( var i = 0; i < optCount; i++) {
		...
		if (defValue == val) {
			retIndex = i;
		} else if (retIndex == -1 && unSelectedVal == val) {
			unSelectedIndex = i;
		}
	}
...
/**
 * var initTrackInfo = { id : "hotspot-1", //初始相对于图片的位置(rect，类似于 image-map) left : 210, top : 1010, width : 236, height : 78, //图片大小信息（像素） refId : "image-1",
