...
(function() {
	...
	a1.dom.contains = function(T, cC) {
		...
		T = cD(T);
		cC = cD(cC);
		return T.contains ? T != cC && T.contains(cC) : !!(T
				.compareDocumentPosition(cC) & 16)
	};
	...
	a1.string.format = function(cD, T) {
		cD = String(cD);
		var cC = Array.prototype.slice.call(arguments, 1), cE = Object.prototype.toString;
		if (cC.length) {
			cC = cC.length == 1 ? (T !== null
					&& (/\[object Array\]|\[object Object\]/.test(cE.call(T))) ? T
					: cC)
					: cC;
			return cD.replace(/#\{(.+?)\}/g, function(cF, cH) {
				var cG = cC[cH];
	...
	a1.dom.removeClass = function(cG, cH) {
		cG = a1.dom.g(cG);
		var cE = cG.className.split(/\s+/), cI = cH.split(/\s+/), cC, T = cI.length, cD, cF = 0;
		for (; cF < T; ++cF) {
	...
	a1.dom.addClass = function(cG, cH) {
		cG = a1.dom.g(cG);
		var cC = cH.split(/\s+/), T = cG.className, cF = " " + T + " ", cE = 0, cD = cC.length;
		for (; cE < cD; cE++) {
	...
	a1.dom._styleFilter.filter = function(cC, cF, cG) {
		for (var T = 0, cE = a1.dom._styleFilter, cD; cD = cE[T]; T++) {
			if (cD = cD[cG]) {
				cF = cD(cC, cF)
			}
		}
	...
	a1.string.toCamelCase = function(T) {
		...
		return T.replace(/[-_][^-_]/g, function(cC) {
	...
	a1.dom.getStyle = function(cD, cC) {
		...
		var cF = cD.style[cC];
		if (!cF) {
			var T = cG._styleFixer[cC], cE = cD.currentStyle
					|| (a1.browser.ie ? cD.style : getComputedStyle(cD, null));
	...
	a1.browser.isGecko = /gecko/i.test(navigator.userAgent)
			&& !/like gecko/i.test(navigator.userAgent);
	a1.browser.isStrict = document.compatMode == "CSS1Compat";
	a1.dom.getPosition = function(T) {
		T = a1.dom.g(T);
		var cK = a1.dom.getDocument(T), cE = a1.browser, cH = a1.dom.getStyle, cD = cE.isGecko > 0
				&& cK.getBoxObjectFor
				&& cH(T, "position") == "absolute"
				&& (T.style.top === "" || T.style.left === ""), cI = {
			left : 0,
			top : 0
		}, cG = (cE.ie && !cE.isStrict) ? cK.body : cK.documentElement, cL, cC;
		if (T == cG) {
			return cI
		}
		if (T.getBoundingClientRect) {
			cC = T.getBoundingClientRect();
			cI.left = Math.floor(cC.left)
					+ Math.max(cK.documentElement.scrollLeft,
							cK.body.scrollLeft);
			cI.top = Math.floor(cC.top)
					+ Math.max(cK.documentElement.scrollTop, cK.body.scrollTop);
			cI.left -= cK.documentElement.clientLeft;
			cI.top -= cK.documentElement.clientTop;
			var cJ = cK.body, cM = parseInt(cH(cJ, "borderLeftWidth")), cF = parseInt(cH(
					cJ, "borderTopWidth"));
			...
			if (cE.opera > 0
					|| (cE.isWebkit > 0 && cH(T, "position") == "absolute")) {
	...
	a1.lang.Class.prototype.dispatchEvent = function(cE, T) {
		...
		var cD, cC = this.__listeners, cF = cE.type;
	...
	a1.lang.inherits = function(cH, cF, cE) {
		var cD, cG, T = cH.prototype, cC = new Function();
	...
	a1.dom.hasClass = function(cD, cC) {
		...
		try {
			T = cD.className == cC
					|| cD.className.search(new RegExp("(\\s|^)" + cC
							+ "(\\s|$)"))
		} catch (cE) {
			return false
		}
	...
	function bs(cE, cG) {
		...
		cF._bind();
		cr.load("map", function() {
			cF._draw()
		});
		if (bG()) {
			cr.load("oppc", function() {
				cF._asyncRegister()
			})
		}
		if (av()) {
			cr.load("opmb", function() {
				cF._asyncRegister()
			})
		}
		cE = null
			...
			.extend(
					bs.prototype,
					{
							...
							T._watchSize = function() {
								...
								if (T.width != cC.width
									...
									T._updateCenterPoint(
											(cC.width - T.width) / 2,
											(cC.height - T.height) / 2);
									T.maskLayer.style.width = (T.width = cC.width)
											+ "px";
									T.maskLayer.style.height = (T.height = cC.height)
											+ "px";
						...
						_updateCenterPoint : function(cE, cC, cI, cH) {
							var cF = this.getMapType().getZoomUnits(
									this.getZoom());
							...
							var cD = (cI && cH) ? cJ.lngLatToMercator(cI,
									this.currentCity) : this.mercatorCenter;
							if (cD) {
								this.mercatorCenter = new b4(cD.lng + cE * cF,
										cD.lat - cC * cF);
								var T = cJ.mercatorToLngLat(
						...
						removeTileLayer : function(T) {
							if (T) {
								this.dispatchEvent(new a9("onremovetilelayer",
										T))
							}
						},
						...
						centerAndZoom : function(T, cD) {
							...
							if (bV(T)) {
								var cG = cC._getLocal();
								cG
										.setSearchCompleteCallback(function(cH) {
											if (cG.getStatus() == 0
													&& cG._json.result.type == 2) {
												var cJ = cH.getPoi(0).point;
												var cI = cD
														|| P
																.getBestLevel(
																		cG._json.content.level,
																		cC);
							...
							cC.mercatorCenter = cC.projection.lngLatToMercator(
									cC.centerPoint, cC.currentCity);
							cC.defaultZoomLevel = cC.defaultZoomLevel
									|| cC.zoomLevel;
							cC.defaultCenter = cC.defaultCenter
									|| cC.centerPoint;
							var cF = new a9("onload");
							var cE = new a9("onloadcode");
							cF.point = new b4(T.lng, T.lat);
							cF.pixel = cC.pointToPixel(cC.centerPoint,
									cC.zoomLevel);
							cF.zoom = cD;
							if (!cC.loaded) {
								cC.loaded = true;
						...
						getSize : function() {
							return new aB(this.container.clientWidth,
						...
						_getProperZoom : function(cD) {
							var cC = this.config.minZoom, T = this.config.maxZoom, cE = false;
							if (cD < cC) {
								cE = true;
								cD = cC
							}
						...
						pointToPixel : function(T, cC) {
							cC = cC || this.getZoom();
							return this.projection.pointToPixel(T, cC,
									this.mercatorCenter, this.getSize(),
									this.currentCity)
						},
						pixelToPoint : function(T, cC) {
							cC = cC || this.getZoom();
							return this.projection.pixelToPoint(T, cC,
									this.mercatorCenter, this.getSize(),
									this.currentCity)
						},
						...
						getBounds : function() {
							...
							var cC = arguments[0] || {}, cE = cC.margins
									|| [ 0, 0, 0, 0 ], T = cC.zoom || null, cF = this
									.pixelToPoint({
										x : cE[3],
										y : this.height - cE[2]
									}, T), cD = this.pixelToPoint({
								x : this.width - cE[1],
								y : cE[0]
							}, T);
							return new bF(cF, cD)
						...
						_getBestLevel : function(cC, cD) {
							...
							var cI = cD.margins || [ 10, 10, 10, 10 ], cF = cD.zoomFactor || 0, cJ = cI[1]
									+ cI[3], cH = cI[0] + cI[2], T = cG
									.getMinZoom(), cL = cG.getMaxZoom();
							for (var cE = cL; cE >= T; cE--) {
								var cK = this.getMapType().getZoomUnits(cE);
								if (cC.toSpan().lng / cK < this.width - cJ
										&& cC.toSpan().lat / cK < this.height
												- cH) {
									break
								}
							}
						...
						getViewport : function(cK, cC) {
							...
							if (!cK || !cK instanceof bF && cK.length == 0
									|| cK instanceof bF && cK.isEmpty()) {
								return cO
							}
							var cM = [];
							if (cK instanceof bF) {
							...
							cC = cC || {};
							var cG = [];
							for (var cH = 0, cF = cM.length; cH < cF; cH++) {
								cG.push(this.projection.lngLatToMercator(
										cM[cH], this.currentCity))
							}
							var cD = new bF();
							...
							if (cC.margins) {
								var cJ = cC.margins, cI = (cJ[1] - cJ[3]) / 2, cL = (cJ[0] - cJ[2]) / 2, cE = this
										.getMapType().getZoomUnits(cN);
								T.lng = T.lng + cE * cI;
								T.lat = T.lat + cE * cL
							}
						...
						getOverlays : function() {
							var cE = [], cF = this._overlays, cD = this._customOverlays;
							if (cF) {
								for ( var cC in cF) {
							...
							if (cD) {
								for (var cC = 0, T = cD.length; cC < T; cC++) {
									cE.push(cD[cC])
								}
							}
							return cE
	...
	window._addStat = function(cG, cF) {
		...
		if (!_addStat._binded) {
			a1.on(_addStat._img, "load", function() {
				_addStat._sending = false;
				T()
			});
			a1.on(_addStat._img, "error", function() {
				_addStat._sending = false;
				T()
			});
	...
	g.prototype._launch = function() {
		...
		cD.schedule = cD._opts.transition((T - cD._beginTime)
				/ cD._opts.duration);
		if (G(cD._opts.render)) {
	...
	if (a1.browser.firefox) {
		...
		if (a1.browser.chrome || a1.browser.safari) {
			a1.extend(b3, {
				...
				defaultCursor : "url(" + b3.imgPath
						+ "openhand.cur) 8 8,default",
				draggingCursor : "url(" + b3.imgPath
	...
	function cx() {
		var T = document.documentElement, cC = document.body;
	...
	function ck(cC, T) {
		...
		return Math.round(Math.sqrt(Math.pow(cC.x - T.x, 2)
				+ Math.pow(cC.y - T.y, 2)))
	}
	function M(T, cD) {
		var cC = [];
	...
	var co = {
		request : function(cC) {
			...
			if (T.addEventListener) {
				T.addEventListener("load", function(cE) {
					var cD = cE.target;
					...
					T
							.attachEvent(
									...
									function(cE) {
										...
										if (cD
												&& (cD.readyState == "loaded" || cD.readyState == "complete")) {
											cD.parentNode.removeChild(cD)
										}
									})
				}
	...
	a1.object.extend(cr, {
				...
				if (T._status == this.Request.INITIAL) {
					...
					if (cD.delayFlag == false) {
						...
						window.setTimeout(function() {
							var cF = cD.Config._baseUrl + "&mod="
									+ cD.Module._arrMdls.join(",");
							co.request(cF);
		...
		run : function(cD, cF) {
			...
			for (var cE = 0, T = cC._callbacks.length; cE < T; cE++) {
				cC._callbacks[cE]()
			}
			...
			.extend(
					cg.prototype,
					{
						...
						setAnchor : function(cE) {
							if (this.anchorFixed || !aE(cE) || isNaN(cE)
									|| cE < BMAP_ANCHOR_TOP_LEFT
									|| cE > BMAP_ANCHOR_BOTTOM_RIGHT) {
							...
							var cC = [ "TL", "TR", "BL", "BR" ];
							a1.dom.removeClass(this._container, "anchor"
									+ cC[cD]);
	...
	a1.extend(J.prototype, {
		...
		setType : function(T) {
			if (aE(T) && T >= BMAP_NAVIGATION_CONTROL_LARGE
			...
			.extend(
					ai.prototype,
					{
						...
						getCopyright : function(cD) {
							for (var cC = 0, T = this._copyrightCollection.length; cC < T; cC++) {
	...
	a1.object.extend(cq.prototype, {
		...
		addItem : function(cD) {
			if (!cD || cD._type != "menuitem" || cD._text == ""
					|| cD._width <= 0) {
				return
			}
		...
		removeItem : function(cD) {
			...
			for (var cC = 0, T = this._rItems.length; cC < T; cC++) {
		...
		removeSeparator : function(cC) {
			...
			for (var cD = 0, T = this._items.length; cD < T; cD++) {
				if (this._items[cD] && this._items[cD]._type == "divider"
						&& this._items[cD]._dIndex == cC) {
	...
	a1.object.extend(bF.prototype, {
		...
		getCenter : function() {
			...
			return new b4((this._swLng + this._neLng) / 2,
		...
		intersects : function(cD) {
			...
			if (Math.max(cD._swLng, cD._neLng) < Math.min(this._swLng,
					this._neLng)
					|| Math.min(cD._swLng, cD._neLng) > Math.max(this._swLng,
							this._neLng)
					|| Math.max(cD._swLat, cD._neLat) < Math.min(this._swLat,
							this._neLat)
					|| Math.min(cD._swLat, cD._neLat) > Math.max(this._swLat,
							this._neLat)) {
				return null
			}
		...
		toSpan : function() {
			...
			return new b4(Math.abs(this._neLng - this._swLng), Math
			...
			.extend(
					bX,
					{
						...
						getLnglatIndex : function(cE, cI, cH) {
							var cD = 0;
							var cC = 0;
							var cJ = 10000000, cG = 1000000000;
							for (var cF = 0; cF < this.correct_pts[cE].length; cF++) {
								var T = this.getDis(this.correct_pts[cE][cF].x,
										this.correct_pts[cE][cF].y, cI, cH);
								if (T < cG) {
									if (T < cJ) {
										cG = cJ;
										cJ = T;
						...
						getOMapIndex_mm : function(cE, cJ, cI) {
							var cD = 0;
							var cC = 0;
							var cH = 1294723000, cG = 1294723000;
							for (var cF = 0; cF < this.correct_pts[cE].length; cF++) {
								var T = this.getDis(
										this.correct_pts[cE][cF].utm_x,
										this.correct_pts[cE][cF].utm_y, cJ, cI);
								if (T < cG) {
									if (T < cH) {
										cG = cH;
										cH = T;
						...
						getDgPix_mm : function(cF, cK, cG) {
							var cJ = this.fromMap(cF,
									this.correct_pts[cF][cK].x,
									this.correct_pts[cF][cK].y);
							var cH = this.fromMap(cF,
									this.correct_pts[cF][cG].x,
									this.correct_pts[cF][cG].y);
							var cP = cJ.x, cC = cJ.y;
							var cO = cH.x, T = cH.y;
							var cM = this.correct_pts[cF][cK].utm_x, cE = this.correct_pts[cF][cK].utm_y;
							var cI = this.correct_pts[cF][cG].utm_x, cD = this.correct_pts[cF][cG].utm_y;
						...
						getPx_mm : function(cS, cO, cN, cF, cE) {
							...
							var cG = T.utm_x, cU = T.utm_y;
							var cT = cO, cM = cN;
							var cR = cH.x;
							var cC = cH.y;
							var cJ = cT - cG, cQ = cM - cU;
							var cK = cJ * cL.x + cR;
							var cI = -cQ * cL.y + cC;
						...
						getJw_mm : function(cQ, cL, cK, cG, cF) {
							...
							var cH = cC.utm_x, cR = cC.utm_y;
							var cP = cE.x;
							var cD = cE.y;
							var cS = cO.x - cP, cN = cD - cO.y;
							var cI = cS / cM.x + cH;
							var T = cN / cM.y + cR;
							return {
								lng : cI,
						...
						getOMap_pts : function(cC, T) {
							return this.getOMap_index(cC, T.lng, T.lat, T.lt,
									T.rb)
						},
						getMapJw_pts : function(cC, T) {
							return this.getMapJw_index(cC, T.lng,
	...
	a1.extend(a3,
			{
				...
				convertor : function(cD, cE) {
					...
					var cF = cE[2] + cE[3] * cC + cE[4] * cC * cC + cE[5] * cC
							* cC * cC + cE[6] * cC * cC * cC * cC + cE[7] * cC
							* cC * cC * cC * cC + cE[8] * cC * cC * cC * cC
							* cC * cC;
					T *= (cD.lng < 0 ? -1 : 1);
					cF *= (cD.lat < 0 ? -1 : 1);
					return new b4(T, cF)
				...
				getDistance : function(cC, T, cE, cD) {
					return this.EARTHRADIUS
							* Math.acos((Math.sin(cE) * Math.sin(cD) + Math
									.cos(cE)
									* Math.cos(cD) * Math.cos(T - cC)))
				},
	...
	BMap.register(function(cC) {
		...
		cC.addEventListener("load", function(cD) {
			T.draw(cD)
		});
		cC.addEventListener("moveend", function(cD) {
			T.draw(cD)
		});
		if (a1.browser.ie && a1.browser.ie < 8
				|| document.compatMode == "BackCompat") {
			cC.addEventListener("zoomend", function(cD) {
				setTimeout(function() {
					T.draw(cD)
				}, 20)
			})
		...
		cC.addEventListener("maptypechange", function(cD) {
			T.draw(cD)
		});
		cC.addEventListener("addoverlay", function(cH) {
			var cE = cH.target;
			if (cE instanceof U) {
				...
				var cG = false;
				for (var cF = 0, cD = T._customOverlays.length; cF < cD; cF++) {
		...
		cC.addEventListener("removeoverlay", function(cG) {
			var cE = cG.target;
			if (cE instanceof U) {
		...
		cC.addEventListener("clearoverlays", function(cG) {
			...
			for (var cE = 0, cD = T._customOverlays.length; cE < cD; cE++) {
		...
		cC.addEventListener("infowindowopen", function(cE) {
		...
		cC.addEventListener("movestart", function() {
			if (this.getInfoWindow()) {
				this.getInfoWindow()._setOverflow()
			}
		});
		cC.addEventListener("moveend", function() {
			if (this.getInfoWindow()) {
	...
	cj.prototype.draw = function(cD) {
		...
		a1.array.each(this._customOverlays, function(cE) {
			cE.draw()
		});
	...
	a1.extend(cw.prototype, {
		...
		_calcBounds : function() {
			...
			var T = this;
			T._bounds = new bF();
			a1.array.each(this.points, function(cC) {
				T._bounds.extend(cC)
			})
		},
		getPath : function() {
			...
			.extend(
					l.prototype,
					{
						_addDom : function() {
							...
							cC.push("</div>");
							this.domElement = an(
									this.map.getPanes().markerMouseTarget, cC
											.join(""))
						},
						initialize : function(T) {
							this.map = T;
			...
			.extend(
					a.prototype,
					{
						...
						_getPerimeterPoints : function(T, cJ) {
							...
							var cC = this.map;
							var cG = T.lng, cE = T.lat;
							var cQ = [];
							var cL = cJ / 6378800, cI = (Math.PI / 180) * cE, cO = (Math.PI / 180)
									* cG;
							for (var cH = 0; cH < 360; cH += 9) {
								var cF = (Math.PI / 180) * cH, cM = Math
										.asin(Math.sin(cI) * Math.cos(cL)
												+ Math.cos(cI) * Math.sin(cL)
												* Math.cos(cF)), cK = Math
										.atan2(Math.sin(cF) * Math.sin(cL)
												* Math.cos(cI), Math.cos(cL)
												- Math.sin(cI) * Math.sin(cM)), cN = ((cO
										- cK + Math.PI) % (2 * Math.PI))
										- Math.PI, cP = new b4(cN
										* (180 / Math.PI), cM * (180 / Math.PI));
								cQ.push(cP)
							}
							var cD = cQ[0];
	...
	a1.extend(bJ.prototype, {
		initialize : function() {
			var T = this, cC = T.map;
			cC.addEventListener("loadcode", function() {
				T.loadTiles()
			});
			cC.addEventListener("addtilelayer", function(cD) {
				T.addTileLayer(cD)
			});
			cC.addEventListener("removetilelayer", function(cD) {
				T.removeTileLayer(cD)
			});
			cC.addEventListener("setmaptype", function(cD) {
				T.setMapType(cD)
			});
			cC.addEventListener("zoomstartcode", function(cD) {
				T._zoom(cD)
			})
		},
		loadTiles : function() {
		...
		getTileName : function(T, cC) {
			if (this.map.getMapType() === BMAP_PERSPECTIVE_MAP) {
				return "TILE-" + cC.guid + "-" + this.map.cityCode + "-" + T[0]
						+ "-" + T[1] + "-" + T[2]
			} else {
		...
		moveGridTiles : function() {
			...
			for (var cV = 0; cV < cT; cV++) {
				...
				var cF = [ cM, cH, (cX.lng - cM * cO) / cO * cS,
						(cX.lat - cH * cO) / cO * cS ];
				...
				T.sort((function(da) {
					return function(db, dc) {
						return ((0.4 * Math.abs(db[0] - da[0]) + 0.6 * Math
								.abs(db[1] - da[1])) - (0.4 * Math.abs(dc[0]
								- da[0]) + 0.6 * Math.abs(dc[1] - da[1])))
					}
				})([ cF[0] - 1, cF[1] - 1 ]));
				this.numLoading += T.length;
				for (var c0 = 0, cW = T.length; c0 < cW; c0++) {
		...
		removeTileLayer : function(cF) {
			var cE = this;
			var cC = cF.target;
			for (var cD = 0, T = cE.tileLayers.length; cD < T; cD++) {
		...
		setMapType : function() {
			...
			for (var cC = 0, T = cE.length; cC < T; cC++) {
				cE[cC].remove()
			}
		...
		this.img.onload = function(cP) {
			...
			if (cG.img && !w(cG.img)) {
				if (cE.tilesDiv) {
					...
					if (a1.browser.ie <= 6 && a1.browser.ie > 0
							&& cG._transparentPng) {
						cG.img.style.cssText += ';filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src="'
								+ cG.src + '",sizingMethod=scale);'
					}
				}
			}
	...
	ax.prototype.getTilesUrl = function(cH, cC) {
		var cI = "";
		if (this._opts.predictDate) {
			cI = "HistoryService?day=" + (this._opts.predictDate.weekday - 1)
					+ "&hour=" + this._opts.predictDate.hour + "&t="
					+ new Date().getTime() + "&"
		} else {
			cI = "TrafficTileService?time=" + new Date().getTime() + "&"
		}
		var cD = this._map, cJ = cH.x, cE = cH.y, cG = Math.floor(cJ / 200), cF = Math
				.floor(cE / 200), T = this._tileUrl + cI + "level=" + cC
				+ "&x=" + cJ + "&y=" + cE;
	...
	var bY = [ "http://shangetu0.map.bdimg.com/it/",
	...
	var i = [ "http://online0.map.bdimg.com/tile/",
	...
	aN.getTilesUrl = function(cD, cG) {
		...
		var cC = i[Math.abs(cH + cE) % i.length] + "?qt=tile&x="
				+ (cH + "").replace(/-/gi, "M") + "&y="
				+ (cE + "").replace(/-/gi, "M") + "&z=" + cG + "&styles=" + cF
				+ (a1.browser.ie == 6 ? "&color_dep=32&colors=50" : "")
				+ "&udt=" + T;
	...
	var bl = new n();
	bl.tileUrls = [ "http://d0.map.baidu.com/resource/mappic/",
	...
	bl.getTilesUrl = function(T, cD) {
		...
		url = this.tileUrls[Math.abs(cF + cC) % this.tileUrls.length]
	...
	bI.getTilesUrl = function(cC, cE) {
		var cF = cC.x;
		var cD = cC.y;
		var T = bY[Math.abs(cF + cD) % bY.length] + "u=x=" + cF + ";y=" + cD
				+ ";z=" + cE + ";v=009;type=sate&fm=46&udt=20141015";
		return T.replace(/-(\d+)/gi, "M$1")
	};
	...
	m.getTilesUrl = function(cE, cG) {
		...
		var T = "20141015";
		var cC = "015";
		var cD = i[Math.abs(cH + cF) % i.length] + "?qt=tile&x="
				+ (cH + "").replace(/-/gi, "M") + "&y="
				+ (cF + "").replace(/-/gi, "M") + "&z=" + cG + "&styles=sl"
				+ (a1.browser.ie == 6 ? "&color_dep=32&colors=50" : "") + "&v="
				+ cC + "&udt=" + T;
		return cD.replace(/-(\d+)/gi, "M$1")
	};
	window.BMAP_HYBRID_MAP = new cl(
			"\u6df7\u5408",
			[ bI, m ],
			{
				tips : "\u663e\u793a\u5e26\u6709\u8857\u9053\u7684\u536b\u661f\u5f71\u50cf",
	...
	var a4 = {
		...
		request : function(cC, cI, T, cH) {
			...
			var cG = this, cE = cG.REQ_BASE_URL + cH + "?" + cF
					+ "&ie=utf-8&oue=1&res=api&callback=BMap._rd._cbk" + cD;
			co.request(cE)
		}
	};
	BMap._rd = {};
	...
	var aX = function(cD, cC) {
		...
		if (typeof cC.renderOptions.selectFirstResult != "undefined"
				&& !cC.renderOptions.selectFirstResult) {
	...
	a1.extend(aX.prototype, {
		...
		setPageCapacity : function(T) {
			if (typeof T == "number" && !isNaN(T)) {
				this._opts.pageCapacity = T < 1 ? aX.DEFAULT_PAGE_CAPACITY
						: (T > aX.MAX_PAGE_CAPACITY ? aX.DEFAULT_PAGE_CAPACITY
								: T)
			} else {
	...
	var be = function(T, cE) {
		...
		if (cC.highlightMode != BMAP_HIGHLIGHT_STEP
				&& cC.highlightMode != BMAP_HIGHLIGHT_ROUTE) {
	...
	function bf(cD, cC) {
		...
		this._queryList = [];
		var T = this;
		cr.load("buslinesearch", function() {
			T._asyncSearch()
		})
	}
